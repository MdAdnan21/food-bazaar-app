...
const DishData = ({ imageId, index }) => (
  <div className="w-36 flex flex-col cursor-pointer rounded-lg justify-center mx-5 my-3 flex-shrink-0 transition-transform transform hover:scale-110 hover:bg-slate-100">
...
const ResData = ({
  ...
  <div className="w-64 flex flex-col p-4 shadow-lg cursor-pointer rounded-lg justify-center mx-6 my-5 flex-shrink-0 transition-transform transform hover:scale-105 hover:bg-slate-100">
    <div className="h-32">
      <img
    ...
    <div>
      <div className="line-clamp-1 py-2">
        <span
          className={`mx-1 pt-[1px] pb-[2px] px-[5px] inline-flex ${getRatingColorClass(
            avgRating
      ...
      <h4 className="pb-1 text-gray-500 mx-1 text-md line-clamp-1">
        {cuisines.join(", ")}
      </h4>
      <div className="flex justify-between">
        <span className="pb-1 mx-1 font-semibold text-gray-600">
          {costForTwo}
        </span>
        <span className="pb-1 mx-1 text-gray-700 font-bold">
          <Time /> {time}
        </span>
      </div>
    </div>
    ...
    <div className="w-full mb-5">
      <div className="flex items-center justify-between text-2xl font-extrabold">
        ...
        <div className="flex items-center mx-5 my-1">
          <button
            ...
            className={`text-xl mx-2 text-gray-800 bg-gray-300 bg-opacity-50 px-2 py-1 rounded-full transition duration-300  hover:bg-slate-200 shadow-lg ${
              isPrevDisabled
                ? "opacity-50 cursor-not-allowed"
                : "hover:text-[#fe8b00] cursor-pointer"
            }`}
          >
            <ArrowLeft />
          </button>
      ...
      >
        {carouselData?.data?.map((data, index) =>
          type === "Dish" ? (
            <DishData key={data?.id} imageId={data?.imageId} index={index} />
          ) : (
